  ...
  try {
    ...
    if (items.length < 10) {
      try {
        ...
        let m;
        while ((m = re.exec(xml)) !== null) {
          if (m[1] && !m[1].includes("YouTube")) items.push(m[1]);
          if (items.length >= 20) break;
        }
      } catch (err) {
    ...
    // Format results
    const formatted = items.slice(0, 20).map((niche, i) => {
      const rank = i + 1;
      const fire = rank <= 5 ? "🔥" : "";
      return `${rank}. ${niche} ${fire}`;
    });
    ...
    const html = `
      ...
      <body>
        <h1>🚀 Top Niches</h1>
        <ul>${formatted.map(f => `<li>${f}</li>`).join("")}</ul>
        <div class="footer">💵 Buckle up... cash incoming soon! | Source: multi</div>
      </body>
      </html>
    `;
    ...
    return { statusCode: 500, headers: { "Content-Type": "text/plain" }, body: "Error fetching niches: " + err.message };
  }
}
